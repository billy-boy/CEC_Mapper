...
{
    public class CEC_KeyMappingList : System.Collections.CollectionBase
    {
        #region Singleton

        private static CEC_KeyMappingList _instance = null;

        public static CEC_KeyMappingList getInstance()
        {
            if (_instance == null) _instance = new CEC_KeyMappingList();
            return _instance;
        }

        #endregion

        public CEC_KeyMappingList()
		{
		}

        public void Add(CEC_KeyMapping item)
		{
            if (!base.InnerList.Contains(item))
                base.InnerList.Add(item);
		}

        public void Remove(CEC_KeyMapping item)
        {
            base.InnerList.Remove(item);
        }

        public bool Contains(CEC_KeyMapping item)
        {
            foreach (CEC_KeyMapping s in base.InnerList)
            {
                if (s == item)
                    return true;
            }
        ...
        public CEC_KeyMapping this[int index]
		{
			get
			{
                return (CEC_KeyMapping)base.InnerList[index];

			}
		}

        public CEC_KeyMapping this[String processName]
        {
            get
            {
                foreach (CEC_KeyMapping s in base.InnerList)
        ...
        private void importEntry(ref CEC_KeyMapping mapping, String section, String name, String value)
        {
            if (mapping == null)
                return;
                ...
                else if (line.Contains("="))
                {
                    ...
                    if (spl.Length > 1)
                    {
                        if (section == "Process" && spl[0].Trim() == "Name")
                        {
                            ...
                            if (mapping == null) mapping = new CEC_KeyMapping(spl[1].Trim());
